...
from pypuppetdb.QueryBuilder import AndOperator, EqualsOperator, OrOperator, RegexOperator, GreaterEqualOperator, \
    LessEqualOperator
...
def reports_ajax(env, node_name):
    ...
    for report in reports_events:
        ...
        for m in puppetboard.metrics.metrics:
...
def report(env, node_name, report_id):
    ...
    return render_template(
        ...
        logs=report.logs,
        metrics=puppetboard.metrics.metrics,
        envs=envs,
        current_env=env)


REPORTS_COLUMNS = [
    {'attr': 'end', 'filter': 'end_time', 'name': 'End time', 'type': 'datetime'},
    {'attr': 'status', 'name': 'Status', 'type': 'status'},
    {'attr': 'certname', 'name': 'Certname', 'type': 'node'},
    {'attr': 'version', 'filter': 'configuration_version', 'name': 'Configuration version'},
    {'attr': 'agent_version', 'filter': 'puppet_version', 'name': 'Agent version'},
]